...
static const char *get_typestr(int *x, int n)
{
   ...
   // TODO: Rewrite these types to used sized integer names:
   // uchar  -> uint8
   // ushort -> uint16
   // uint   -> uint32
   // ????   -> uint64
   ...
   // ????   -> int64
   //
   // But these sizes should only be used when required -- basically
   // for storing data in compact formats.  They are not needed in a
   // program like this, because it's not directly accessing hardware
   // or registers that specific size requirements.  In other words,
   // let the compiler do its job when creating executable code.
                           ...
                           const char *fname)
{
   ...
   fprintf (fp, "{\n");
   fprintf (fp, "      if (lexer.token.end == lexer.token.start)        // Illegal character?\n");
   fprintf (fp, "      {\n");
   fprintf (fp, "         lexer.token.end++;\n");
   fprintf (fp, "      }\n");
   fprintf (fp, "      return 0;\n");
   fprintf (fp, "}\n");
   fprintf (fp, "\n");
   ...
   fprintf (fp, "      if (lexer.lookahead.start > 0)             // In lookahead mode?\n");
   fprintf (fp, "      {\n");
   fprintf (fp, "         sti = add_symbol (t, lexer.lookahead.start, lexer.lookahead.end);\n");
   fprintf (fp, "      }\n");
   fprintf (fp, "      else                                 // Regular mode of parsing.\n");
   fprintf (fp, "      #endif\n");
   fprintf (fp, "      {\n");
   fprintf (fp, "         sti = add_symbol (t, lexer.token.start, lexer.token.end);\n");
   fprintf (fp, "      }\n");
