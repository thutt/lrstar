   ...
   if (!incycle[s])
   {
      ...
      for (t = tt_start[s]; t < tt_start[s+1]; t++)
      {
         ...
         if (traceable[x])
         {
            ...
            {
               trace_top = n_symbols;
               TRACE (x, agoto, n_symbols, n_symbols + 1);
               ++n_symbols;
            }
            traceable[x] = 0;
            l_finalx[x] = n_finalx;
...
void  LG_AddExtra::p_prod2 (int p, int dot, const char *before, const char* after)
{
   int t, u, d, h;
   h = head_sym[p];
...
int   LG_AddExtra::p_sym2 (int s, const char *sp)
{
   char *p;
