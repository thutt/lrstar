...
""")
    ...
    o.add_argument("--lrstar-build-path",
                   help     = "Root of build output directory.",
                   action   = "store",
                   required = True,
                   dest     = "arg_lrstar_build_path")
...
def parse_arguments():
    ...
    options.lrstar_dir = os.environ["LRSTAR_DIR"] # Guaranteed set by wrapper.
    return options
...
def main():
    ...
    modi = gather_artifacts(options, options.arg_lrstar_build_path)
